...
> 作用：简化计算量和约束，例如SO(3)就存在9个量，但是旋转本质就3个自由度（x,y,z)

使用一个向量，其方向与旋转轴一致，而长度等于旋转角。这种向量称为旋转向量(或轴角/角轴， Axis-Angle) ，只需一个三维向量即可描述旋转。

## 罗德里斯公式

$$
R = \cos \theta \, I + (1 - \cos \theta) \, \mathbf{n} \mathbf{n}^T + \sin \theta \, \hat{\mathbf{n}}\\
$$

> 1.**R**: 旋转矩阵，用于表示一个物体绕单位向量 nn 旋转角度 θθ 后的变换。它是一个 3×33×3 的矩阵。
>
> 2.**θ**: 旋转角度，表示旋转的弧度。旋转角度 θθ 是逆时针方向旋转的角度。
>
> 3.**I**: 单位矩阵，即 3×3 的矩阵，形如：
>
> ​	它是矩阵中的恒等元素，用于表示无旋转或不变的部分。
> $$
> I=\begin{bmatrix}
> 1,0,0 \\
> 0,1,0\\
...
> \end{bmatrix}
> $$
> 4.*nn**: 旋转轴的单位向量。它是一个三维列向量，表示旋转的轴方向，并且 ∣n∣=1，即它是一个单位向量。
>
> **nT**:n 的转置，表示 n 向量的行表示。
>
> **n^**: 这是向量 n 的叉乘矩阵，也可以表示为：
> $$
...
> \end{pmatrix}
> $$
> 这个矩阵用于表示 n 与其他向量之间的叉积操作。
> $$
> 给定向量 ( \mathbf{n} = (n_1, n_2, n_3)^T )，其叉乘矩阵 ( \hat{\mathbf{n}} ) 为：
> \\
> \hat{\mathbf{n}} = \begin{pmatrix} 
> 0 & -n_3 & n_2 \\
> n_3 & 0 & -n_1 \\
...
> \end{pmatrix}
> \\
> 假设 ( \mathbf{v} = (v_1, v_2, v_3)^T ) 是另一个向量，则叉积 ( \mathbf{n} \times \mathbf{v} ) 可以通过矩阵乘法得到：
> \\
> \mathbf{n} \times \mathbf{v} = \hat{\mathbf{n}} \mathbf{v}
> \\
> \hat{\mathbf{n}} \mathbf{v} = \begin{pmatrix} 
> 0 & -n_3 & n_2 \\
> n_3 & 0 & -n_1 \\
> -n_2 & n_1 & 0 
> \end{pmatrix} 
...
> 旋转矩阵（正交矩阵） R 作用在向量 n上其等价于数字 1 作用在向量 n上，该特点表示向量 n  就是 R 进行特征分解后，特征值1对应的特征向量，求得该特征向量再进行归一化即可。

# g2o(General Graphic Optimization)图优化

<img src="img/004.png">

在某个时刻 t 的位姿（pose→上图三角形）就是一个顶点，这个也是待优化的变量。而位姿之间的关系就构成了一个边(上图蓝色实线)，比如时刻 t 和时刻 t+1 之间的相对位姿变换矩阵就是边，边通常表示误差项。在SLAM里，图优化一般分解为两个任务：
...
在没有优化的情况下，左图为最后的位姿构建图，<span style="color:red">误差太大</span>，但有图优化后，就比较接近更好的理论模型。
